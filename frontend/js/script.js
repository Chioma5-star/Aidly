    ...
    try {
        ...
        list.innerHTML = requests.map(r => `
            <div style="border:1px solid #e2e8f0; border-radius:12px; margin-bottom:12px; overflow:hidden; background:white; box-shadow:0 2px 6px rgba(0,0,0,0.04);">
                ${r.imagePath ? `<img src="${window.BACKEND_URL || "https://aidly-q8i6.onrender.com"}${r.imagePath}" style="width:100%; height:140px; object-fit:cover;" onerror="this.style.display='none'">` : ""}
        ...
        if (res.ok) {
            ...
            historyList.innerHTML = donations.map(d => `
                <div style="padding:12px; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between; align-items:center; gap:12px;">
                    ${d.imagePath ? `<img src="${window.BACKEND_URL || "https://aidly-q8i6.onrender.com"}${d.imagePath}" style="width:48px; height:48px; object-fit:cover; border-radius:8px; flex-shrink:0;" onerror="this.style.display='none'">` : `<div style="width:48px; height:48px; background:#f1f5f9; border-radius:8px; display:flex; align-items:center; justify-content:center; font-size:20px; flex-shrink:0;">${d.type === "money" ? "💰" : d.type === "food" ? "🍱" : "👕"}</div>`}
                    <div style="flex:1;">
